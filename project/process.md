...
**Classifier Upgrade:** Phase 1 was upgraded from a Pipit-only comparison to a multi-species classifier. It now builds a reference library of all 19 bird species in the preserve. Each Kasios recording is mathematically compared against the entire library using DTW. The results include a confidence percentage for each species. This allows the dashboard to definitively identify when a recording claimed to be a Pipit actually belongs to another bird (e.g., Orange Pine Plover), providing the 'smoking gun' evidence for the investigation.
